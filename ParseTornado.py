...
class Parser(object):
    def __init__(self, timestamp, imageFileGetter, analyzedFolder, correctDigits=None):
        self.imageFileGetter = imageFileGetter

        self.timestamp = timestamp
        self.dataDir = os.path.join(analyzedFolder, str(self.timestamp))
        ...
        self.correctDigits_ = correctDigits
...
""")
    ...
    def digits(self, forceReparse=False):
        if not forceReparse and self.digits_ is not None:
            return self.digits_
        ...
        try:
            ...
            buff = io.BytesIO(self.imageFileGetter().read())
            image = Image.open(buff)
            self.digits_ = parse(self, image)
        except KeyboardInterrupt:
            # We special case KeyboardInterrupt, because this exception is
            # generated by the user. All other exceptions are logged into
            # the html file, and we continue chugging along.
            raise
        except:
            self.digits_ = [ None ]
            einfo = sys.exc_info()
    ...
    def failedTest(self):
        assert self.digits_ is not None
        if self.correctDigits_ is not None:
...
def main():
    analyzedFolder = '/home/jeremy/tmp/'
    fileName = "/home/jeremy/Dropbox/Apps/Tornado Tracker/1366873792.jpg"

    timestamp, correctDigits = splitTimestampAndCorrect(fileName)
    parser = Parser(timestamp, lambda: fileName, analyzedFolder, correctDigits=correctDigits)
